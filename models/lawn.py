...
def generate_lawn_id():
    chars = string.ascii_uppercase + string.digits
    code = "".join(random.choices(chars, k=6))
    return f"RBV-LWN-{code}"


class LawnBooking(db.Model):
    __tablename__ = "lawn_bookings"

    id = db.Column(db.Integer, primary_key=True)
    booking_id = db.Column(db.String(20), unique=True, nullable=False, default=generate_lawn_id)
    event_type = db.Column(db.String(50), nullable=False)
    event_date = db.Column(db.Date, nullable=False)
    event_time = db.Column(db.String(50), default="")
    guest_count = db.Column(db.Integer, nullable=False)
    veg_nonveg = db.Column(db.String(20), default="veg")
    ...
    def __repr__(self):
        return f"<LawnBooking {self.booking_id}>"
